  ...
  return (
    <div className='w-[526px] flex flex-col'>
      ...
      <div className='relative mb-4 w-[526px] h-[480.66px] pr-[23.67px] pl-[23.67px] border rounded-[10px] border-[#00000014] flex justify-center overflow-hidden'>
        <Image src={mainImage} alt={product?.product.title} fill sizes='50vw' className='rounded-lg object-cover' />
      </div>
        ...
        <div className='overflow-hidden w-full' ref={emblaRef}>
          <div className='flex gap-[13.6px]'>
            {[...product?.product.images].map((img: string, index: number) => (
              ...
              >
                <Image src={img} alt={`Product Image ${index + 1}`} width={120.96} height={120.97} className='object-cover' />
              </div>
            ))}
          </div>
        </div>
